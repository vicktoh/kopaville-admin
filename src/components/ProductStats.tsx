  ...
  return (
    <Box bg="brand.100" borderRadius="lg" p={5}>
      <HStack alignItems="center">
        {icon}
        <Heading fontSize="lg">{roundNumbers(count)}</Heading>
      </HStack>
...
export const ProductStats: FC = () => {
  ...
  return isLoadingStats && !stats ? (
    ...
    <HStack spacing={8}>
      <StatBox
        title="Total Product"
        icon={<BsGiftFill color="#2A9A4A" size={32} />}
        count={stats?.productCount || 0}
      />
      <StatBox
        title="Categories"
        icon={<FaCodeBranch color="#2A9A4A" size={32} />}
        count={stats?.categoriesCount || 0}
      />
      <StatBox
        title="Product Value"
        icon={<FaMoneyBillAlt color="#2A9A4A" size={32} />}
        count={stats?.productValue || 0}
      />
    </HStack>
  );
};
